...
class QuestionBuilderForm(forms.Form):
    ...
    sources = forms.CharField(label='Sources', widget=forms.Textarea, help_text='Paste your '
                                                                                'sources (Links or Text) here')
...
class QBuilderUpdateForm(forms.Form):
    ...
    def empty_init(self, *args, **kwargs):
        ...
        self.fields['question_title'] = forms.CharField(label='Question Title', max_length=75,
                                                        initial=data['question_title'], widget=forms.HiddenInput())
        self.fields['num_primary_keywords'] = forms.IntegerField(label='', initial=primary_keywords,
                                                                 widget=forms.HiddenInput())
        self.fields['num_secondary_keywords'] = forms.IntegerField(label='', initial=secondary_keywords,
                                                                   widget=forms.HiddenInput())
        ...
        for index in range(secondary_keywords):
            self.fields['secondary_keyword_field_{index}'.format(index=index)] = forms.CharField(
                label='', initial=data['secondary_keyword_field_{index}'.format(index=index)],
                widget=forms.HiddenInput())

            self.fields['secondary_keyword_point_field_{index}'.format(index=index)] = forms.IntegerField(
                label='', initial=data['secondary_keyword_point_field_{index}'.format(index=index)],
                widget=forms.HiddenInput())
